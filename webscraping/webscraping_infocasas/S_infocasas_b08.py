...
for i in range(5):
    ...
    if i > 0:
        lst_article2 = driver.find_elements(By.XPATH, '//div[@id="__next"]/div[3]//div[1]/a[@class="lc-data"]')
        main_window = driver.window_handles[0]
        ...
        try:
         ...
         # creo una lista con los article de las urls para que no pierda la informacion al abrir un nuevo tab
        urls_alquiler = []
        for alquiler in lst_article2:
            url_alquiler = alquiler.get_attribute('href')
            urls_alquiler.append(url_alquiler)

        for i, article in enumerate(lst_article2):
            # abre una pestaña nueva
            article.click()
            time.sleep(1)
